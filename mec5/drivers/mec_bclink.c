...
#if 0 /* do not implement polling in HAL */
/* BC-Link read operation
 * 1. If BUSY retturn MEC_RET_ERR_BUSY
 * 2. Write address to access in companion device to device address register
 * 3. Read Data register to trigger transmit of read packet to compantion de
 *    Data is discarded.
 * 4. When controller receives response it clears BUSY and sets BUSY Clear Interrupt.
 *    If CRC check failed it also sets BERR status which if enabled generates BERR interrupt.
 * 5. If no error data register can be read to obtain companion's valid data response.
 */
int mec_hal_bcl_read_polled(struct mec_bcl_regs *regs, uint8_t target_address, uint8_t *data)
{
    uint32_t hsts = 0, temp = 0;
    ...
    regs->STATUS |= MEC_BIT(MEC_BCL_STATUS_BCERR_Pos);
    regs->DESTA = target_address;
    /* first read of data triggers transmit of read request */
    temp = regs->DATA;
    /* wait for BUSY to clear. NOTE: BC-Link will generate either BCLR or BERR interrupt */
    hsts = regs->STATUS;
    while (hsts & MEC_BIT(MEC_BCL_STATUS_BUSY_Pos)) {
        hsts = regs->STATUS;
    }

    /* Check if BERR */
    if (hsts & MEC_BIT(MEC_BCL_STATUS_BCERR_Pos)) {
        regs->STATUS |= MEC_BIT(MEC_BCL_STATUS_BCERR_Pos);
        return MEC_RET_ERR;
    }
...
int mec_hal_bcl_write_polled(struct mec_bcl_regs *regs, uint8_t target_address, uint8_t data)
{
    uint32_t hsts = 0;
    ...
    regs->STATUS |= MEC_BIT(MEC_BCL_STATUS_BCERR_Pos);
    regs->DESTA = target_address;
    /* write of data triggers transmit to companion device */
    regs->DATA = data;
