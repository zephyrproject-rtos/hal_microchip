...
#define MEC_ESPI_OOB_UP_ECIA_INFO MEC5_ECIA_INFO(MEC5_ESPI_GIRQ, MEC5_ESPI_OOB_UP_GIRQ_POS, \
                                                 MEC5_ESPI_GIRQ_AGGR_NVIC, MEC5_ESPI_OOB_UP_NVIC_DIRECT)
#define MEC_ESPI_OOB_DN_ECIA_INFO MEC5_ECIA_INFO(MEC5_ESPI_GIRQ, MEC5_ESPI_OOB_DN_GIRQ_POS, \
                                                 MEC5_ESPI_GIRQ_AGGR_NVIC, MEC5_ESPI_OOB_DN_NVIC_DIRECT)

#define MEC_ESPI_OOB_TX_STS_RW1C \
    (ESPI_IO_OOBTXSTS_DONE_Msk | ESPI_IO_OOBTXSTS_CHEN_CHG_Msk | ESPI_IO_OOBTXSTS_EC_BUS_ERR_Msk \
     | ESPI_IO_OOBTXSTS_START_OVRUN_Msk | ESPI_IO_OOBTXSTS_BAD_REQ_Msk)

#define MEC_ESPI_OOB_RX_STS_RW1C \
    (ESPI_IO_OOBRXSTS_DONE_Msk | ESPI_IO_OOBRXSTS_EC_BUS_ERR_Msk | ESPI_IO_OOBRXSTS_DATA_OVRUN_Msk)
...
void mec_espi_oob_girq_ctrl(uint8_t enable, uint8_t msk)
{
    ...
    mec_girq_bm_en(MEC5_ESPI_GIRQ, bitmap, enable);
}
...
void mec_espi_oob_girq_status_clr(uint8_t msk)
{
    ...
    mec_girq_bm_clr_src(MEC5_ESPI_GIRQ, bitmap);
}

uint32_t mec_espi_oob_girq_status(void)
{
    uint32_t src = mec_girq_source_get(MEC5_ESPI_GIRQ);
...
{
    uint32_t result = mec_girq_result_get(MEC5_ESPI_GIRQ);
...
uint32_t mec_espi_oob_max_pkt_size(struct espi_io_regs *iobase)
{
    ...
    sz = (iobase->OOBRXC & ESPI_IO_OOBRXC_MAX_PLD_SZ_Msk) >> ESPI_IO_OOBRXC_MAX_PLD_SZ_Pos;
...
int mec_espi_oob_buffer_set(struct espi_io_regs *iobase, uint8_t dir, struct mec_espi_oob_buf * buf)
{
    ...
    maxlen = mec_espi_oob_max_pkt_size(iobase);
    lenb = buf->len;
    if (lenb > maxlen) {
        lenb = maxlen;
    }
    ...
    if (dir == MEC_ESPI_OOB_DIR_DN) {
        ...
        temp = iobase->OOBRXL & ~(ESPI_IO_OOBRXL_RX_BUF_LEN_Msk);
        temp |= ((lenb << ESPI_IO_OOBRXL_RX_BUF_LEN_Pos) & ESPI_IO_OOBRXL_RX_BUF_LEN_Msk);
        iobase->OOBRXL = temp;
    } else {
        iobase->OOBTXA = buf->maddr;
        temp = iobase->OOBTXL & ~(ESPI_IO_OOBTXL_TX_MSG_LEN_Msk);
        temp |= ((lenb << ESPI_IO_OOBTXL_TX_MSG_LEN_Pos) & ESPI_IO_OOBTXL_TX_MSG_LEN_Msk);
        iobase->OOBTXL = temp;
    }
...
{
    uint32_t tag = ((iobase->OOBRXSTS & ESPI_IO_OOBRXSTS_OOB_RX_TAG_Msk) >>
                    ESPI_IO_OOBRXSTS_OOB_RX_TAG_Pos);
...
uint32_t mec_espi_oob_received_len(struct espi_io_regs *iobase)
{
    ...
    if (iobase) {
        recvlen = iobase->OOBRXL & ESPI_IO_OOBRXL_RECV_MSG_LEN_Msk;
        recvlen >>= ESPI_IO_OOBRXL_RECV_MSG_LEN_Pos;
    }
...
{
    uint8_t done_pos = ESPI_IO_OOBTXSTS_DONE_Pos;
...
{
    uint32_t msk = (ESPI_IO_OOBTXSTS_EC_BUS_ERR_Msk
                    | ESPI_IO_OOBTXSTS_START_OVRUN_Msk
                    | ESPI_IO_OOBTXSTS_BAD_REQ_Msk);

    if (dir == MEC_ESPI_OOB_DIR_DN) {
        msk = (ESPI_IO_OOBRXSTS_EC_BUS_ERR_Msk | ESPI_IO_OOBRXSTS_DATA_OVRUN_Msk);
    }
...
{
    if (dir == MEC_ESPI_OOB_DIR_UP) {
        iobase->OOBTXSTS = (ESPI_IO_OOBTXSTS_EC_BUS_ERR_Msk | ESPI_IO_OOBTXSTS_START_OVRUN_Msk
                            | ESPI_IO_OOBTXSTS_BAD_REQ_Msk);
    } else {
        iobase->OOBRXSTS = (ESPI_IO_OOBRXSTS_EC_BUS_ERR_Msk | ESPI_IO_OOBRXSTS_DATA_OVRUN_Msk);
    }
}
