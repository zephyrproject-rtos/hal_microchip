...
/**
 * @file qtm_acq_pic32cmjh_0x002f_api.h
 ...
 * for Microchip touch support. These headers map the device-specific acquisition
 * module to generic names used by the common driver.
 */

#ifndef TOUCH_API_CMJH_ACQ_H
#define TOUCH_API_CMJH_ACQ_H
...
* OR
* FREQ_SEL_SPREAD automatically sweeps through all 15 delay values during
...
/* Acquisition module PIC32CMJH Sequential */
typedef struct {
...
} qtm_acq_pic32cmjh_node_config_t;
...
typedef struct {
	...
	qtm_acq_pic32cmjh_node_config_t(*qtm_acq_node_config);
...
typedef void (*qtm_drivenshield_callback_t)(uint8_t csd, uint8_t sds, uint8_t prescaler,
					    uint32_t volatile *ptr, uint32_t value);
...
/*
 * touch_ret_t qtm_calibrate_sensor_node(ptc_seq_acq_settings* qtm_acq_control_l_ptr,
 ...
 *
 * Purpose: Configures the PTC for sleep mode measurement of a single node,
 *			with window comparator wake
...
/*
 * uint16_t qtm_pic32cmjh_acq_module_get_id(void);
 *
...
/*
 * uint8_t qtm_pic32cmjh_acq_module_get_version(void);
 *
...
/*
 * void qtm_pic32cmjh_ptc_handler_eoc(void)
 *
